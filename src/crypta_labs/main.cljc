(ns crypta_labs.main
  "Kotodama WASM entrypoint for the Crypta_labs clean-room actor (L4) — Clojure port.
  ...
  facts (`crypta_labs.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "crypta_labs")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"crypta_labs.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "crypta_labs-compat" :tier tier :entities entities} 200])
...
(defn start! [] :crypta_labs-compat/ready)
